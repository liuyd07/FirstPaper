...
Synchronization requirements has long been a main problem of 
Cyber-physical energy system works
...
\subsection{Pulse-Coupled Oscillator Methods}
...
\end{equation}
for $i = 1, 2,..., N$, where ${{\theta }_{i}}\in \left[ 0,2\pi  \right)$ denote the phases of the oscillator $i$. Each phase $\theta_{i}$ corresponds with a state variable $x_i$. The relation between $Q(x)$ and $f(x)$ is \cite{Neuroscience_2007}:
\begin{equation}
\label{Qx_Fx}
Q(\theta_{j} - \theta_{i}) = \frac{1}{T}\int_{0}^{T}f_{i}(x_{i}(t))\delta(t - t_j)dt
\end{equation}
...
\begin{theorem}
Under Assumption 3, the oscillator network described in (\ref{PCO_phase_dynamics_deviate_network}) will either synchronizes or convergence to stable phase locking state.
\end{theorem}

\begin{IEEEproof}
Construct Lyapunov function $V = (1/2)\varphi^{T}\varphi$. $V$ is non-negative and will be zero only if all the $\phi_{i} = 0$, which indicates all the node in the network synchronized. Thus, differentiating $V$ yields:
\begin{equation}
\label{proof1}
\begin{split}
...
According Assumption 3,$\forall\phi_{i}\in(-\pi,\pi)$, we have $Q\left(x\right)/x>0 $. For diagonal matrix, eigenvalues follows:
\begin{equation}
\label{eigenvalue}
\begin{split}
...
\begin{equation}
\label{conv_rate_2}
\alpha =\varepsilon\lambda_{min}\boldsymbol{B}\boldsymbol{B}^{T} = \varepsilon\lambda_{0}\boldsymbol{B}\boldsymbol{B}^{T}
\end{equation}
...
\subsection{Advanced Delay Tanh Model}
...
Substitute $Q(x)$ in (\ref{Quadratic Polygon Model4}) to (\ref{conv_rate_2}), the convergence rate would be:

\begin{equation}
\label{interaction between para k and coupling strength}
\begin{split}
{\alpha }&=\left( {{\sigma }_{1}}{{\sigma }_{2}}\varepsilon \mathbf{B}{{\mathbf{B}}^{T}} \right)\\
&=\min \left( \pi -x \right)k\varepsilon \mathbf{B}{{\mathbf{B}}^{T}}
\end{split}
\end{equation}
...
H. K. Khalil, "Nonlinear Systems", Englewood Cliffs, NJ: Prentice-Hall, 2002, pp. 1-42.
