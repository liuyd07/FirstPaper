...
As a fundamental service, fault diagnosis methods make requirements on the synchronization accuracy. IEC Standard 61588 \cite{IEC61588} and implementation of GPS has provide precise time stamping of the measuring data. Nevertheless, on the demand side the expense of PMU and GPS is high. Smart meters at the utilization side do not afford complicated communication wires.
Sufficient synchronous accuracy for distributed wireless network is urgent\cite{WSN_In_SmartGrid}.
...
From (\ref{PCO_phase_dynamics_deviate_network}), the synchronization of weakly coupled network follows nonlinear dynamic system. The convergence of the 
\begin{theorem}
...
To maximize the $Q(\theta)/\theta$, take quadratic polygon function for example, the phase function should have the following form:
\begin{equation}
...
\end{matrix} \right.
\end{equation}
Thus $Q(\theta)/\theta = k(\pi - \theta)$, which increases with the parameter $k$. However, the $Q(\theta)/\theta$ function only explicitly defines the lower-limit of the convergence rate. Simulations are presented to evaluate the novel phase model. 3 Phase models are shown in Fig. \ref{Fig_Qx}, and the convergence criterion $Q(\theta)/\theta$ are shown in Fig. \ref{Fig_Qx_x}.
...
\caption{The convergence rate criterion $Q(\theta)/\theta$ of 3 phase response models in PCO}
\label{Fig_Qx_x}
\end{figure}
...
\bibitem{CPES_Intro}
C.A. Macana, N. Quijano, E. Mojica-Nava. "A survey on Cyber Physical Energy Systems and their applications on smart grids", Innovative Smart Grid Technologies (ISGT) Latin Ame, 2011: 1-7.
